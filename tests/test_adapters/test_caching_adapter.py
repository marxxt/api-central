...
async def test_store_user_stores_in_both():
    ...
    adapter = CachingAdapter(cache=MockCache(), primary=MockPrimary())
    await adapter.store_user(User(id=ID("abc123"), user_id=ID("abc123"), first_name="Log", last_name="Test", email="log@example.com", role="USER", created_at=datetime.utcnow()))
...
async def test_primary_store_fails_but_cache_succeeds():
    ...
    with pytest.raises(RuntimeError, match="Primary write error"):
        await adapter.store_user(User(id=ID("abc123"), user_id=ID("abc123"), first_name="Bad", last_name="Write", email="bad@example.com", role="USER", created_at=datetime.utcnow()))
