...
DROP INDEX IF EXISTS idx_reputations_subject;
...
DROP POLICY IF EXISTS "Users can view their own reputations"
...
DROP POLICY IF EXISTS "Users can create their own reputations"
ON public.reputations;
...
DROP POLICY IF EXISTS "Users can update their own reputations"
ON public.reputations;  
...
DROP POLICY IF EXISTS "Users can delete their own reputations"
ON public.reputations;
...
DROP INDEX IF EXISTS idx_snfts_owner_id;
DROP INDEX IF EXISTS idx_snfts_collection_id;
DROP INDEX IF EXISTS idx_snfts_category;
...
DROP POLICY IF EXISTS "Enable read access for all users" ON public.snfts;
DROP POLICY IF EXISTS "Users can create SNFTs" ON public.snfts;
DROP POLICY IF EXISTS "Users can update their SNFTs" ON public.snfts;
DROP POLICY IF EXISTS "Users can delete their SNFTs" ON public.snfts;
...
DROP INDEX IF EXISTS idx_auctions_property_id;
...
DROP POLICY IF EXISTS "Users can update their own auctions" ON public.auctions;
DROP POLICY IF EXISTS "Users can delete their own auctions" ON public.auctions;
...
-- Add indexes for performance
DROP INDEX IF EXISTS idx_trading_pairs_base_asset_id;
DROP INDEX IF EXISTS idx_trading_pairs_base_asset_symbol;
DROP INDEX IF EXISTS idx_trading_pairs_quote_asset_symbol;
...
DROP POLICY IF EXISTS "Enable read access for all users" ON public.trading_pairs;
...
-- Add indexes for performance
DROP INDEX IF EXISTS idx_ai_bot_configs_user_id;
DROP INDEX IF EXISTS idx_ai_bot_configs_trading_pair_id;
...
DROP POLICY IF EXISTS "Users can view their own bot configs" ON public.ai_bot_configs;
DROP POLICY IF EXISTS "Users can create their own bot configs" ON public.ai_bot_configs;
DROP POLICY IF EXISTS "Users can update their own bot configs" ON public.ai_bot_configs;
DROP POLICY IF EXISTS "Users can delete their own bot configs" ON public.ai_bot_configs;
...
DROP INDEX IF EXISTS idx_orders_user_pair_status;
DROP INDEX IF EXISTS idx_orders_trading_pair_status;
...
DROP INDEX IF EXISTS idx_orders_trading_pair_id;
...
-- Add indexes for performance
DROP INDEX IF EXISTS idx_user_investments_user_id;
DROP INDEX IF EXISTS idx_user_investments_property_asset_id;
...
DROP INDEX IF EXISTS idx_bid_history_auction_id;
...
DROP POLICY IF EXISTS "Enable read access for all users" ON public.bid_history;
DROP POLICY IF EXISTS "Users can create their own bids" ON public.bid_history;
DROP POLICY IF EXISTS "Users can update their own bids" ON public.bid_history;
DROP POLICY IF EXISTS "Users can delete their own bids" ON public.bid_history;
...
DROP POLICY IF EXISTS "Enable read access for all users" ON public.dao_proposals;
DROP POLICY IF EXISTS "Users can create DAO proposals" ON public.dao_proposals;
...
DROP INDEX IF EXISTS idx_dao_votes_proposal_id;
DROP INDEX IF EXISTS idx_dao_votes_user_id;
...
DROP POLICY IF EXISTS "Users can delete their own user stats" ON public.user_stats;
...
CREATE POLICY "Users can delete their own user stats" ON public.user_stats
    FOR DELETE USING (auth.uid() = user_id);

---------------------------------------------------------------------------------
-- 15. Table for Comments
-- Inferred from data/comments.py
---------------------------------------------------------------------------------
...
CREATE TABLE public.comments (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id uuid REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
    asset_type text NOT NULL, -- e.g., 'snft', 'post', 'property', 'bot_strategy', 'chat_message'
    asset_id uuid NOT NULL, -- ID of the asset being commented on
...
CREATE INDEX idx_comments_asset ON public.comments (asset_id, asset_type);
CREATE INDEX idx_comments_user_id ON public.comments (user_id);
CREATE INDEX idx_comments_parent_id ON public.comments (parent_id);

ALTER TABLE public.comments ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Enable read access for all users on comments" ON public.comments;
DROP POLICY IF EXISTS "Users can create their own comments" ON public.comments;
DROP POLICY IF EXISTS "Users can update their own comments" ON public.comments;
DROP POLICY IF EXISTS "Users can delete their own comments" ON public.comments;

CREATE POLICY "Enable read access for all users on comments" ON public.comments
    FOR SELECT USING (TRUE);
...
CREATE TRIGGER set_comments_timestamp
    ...
    FOR EACH ROW
    EXECUTE FUNCTION public.set_update_timestamp();
DROP TABLE IF EXISTS public.snft_stats CASCADE;
...
DROP INDEX IF EXISTS idx_platform_metrics_entity;
DROP INDEX IF EXISTS idx_platform_metrics_entity_id;
...
DROP POLICY IF EXISTS "Enable read access for all users on platform_metrics" ON public.platform_metrics;
DROP POLICY IF EXISTS "Users can insert their own platform metrics" ON public.platform_metrics;
DROP POLICY IF EXISTS "Users can update their own platform metrics" ON public.platform_metrics;
DROP POLICY IF EXISTS "Users can delete their own platform metrics" ON public.platform_metrics;

CREATE POLICY "Enable read access for all users on platform_metrics" ON public.platform_metrics
    FOR SELECT USING (TRUE);

CREATE POLICY "Users can insert their own platform metrics" ON public.platform_metrics
    FOR INSERT WITH CHECK (auth.uid() = entity_id);

CREATE POLICY "Users can update their own platform metrics" ON public.platform_metrics
    FOR UPDATE USING (auth.uid() = entity_id);

CREATE POLICY "Users can delete their own platform metrics" ON public.platform_metrics
...
DROP INDEX IF EXISTS idx_property_wallet_unique;
...
DROP TABLE IF EXISTS public.dao_managers CASCADE;
...
DROP TABLE IF EXISTS public.dao_contractors CASCADE;
...
-- Table for DAO Token Holdings
DROP TABLE IF EXISTS public.dao_token_holdings CASCADE;
...
DROP INDEX IF EXISTS idx_snft_events_snft_id;
DROP INDEX IF EXISTS idx_snft_events_event_type;

CREATE INDEX idx_snft_events_snft_id ON public.snft_events (snft_id);
CREATE INDEX idx_snft_events_event_type ON public.snft_events (event_type);
...
CREATE TRIGGER trg_snft_update_metadata_trigger
...
DROP INDEX IF EXISTS idx_favorites_asset;
...
DROP POLICY IF EXISTS "Users can favorite their own favorites" ON public.favorites;
DROP POLICY IF EXISTS "Users can unfavorite their own favorites" ON public.favorites;
...
CREATE TABLE public.shares (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id uuid REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
    asset_type text NOT NULL, -- e.g., 'snft', 'post', 'property', 'bot_strategy'
    asset_id uuid NOT NULL, -- ID of the asset being shared
    shared_with text, -- e.g., 'email', 'social_media', 'direct_message'
    comment text,
    shared_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone -- Added for consistency
);

CREATE INDEX idx_shares_asset ON public.shares (asset_id, asset_type);
...
CREATE POLICY "Users can share assets" ON public.shares
    FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their shares" ON public.shares
    FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Users can delete their shares" ON public.shares
    FOR DELETE USING (auth.uid() = user_id);

CREATE TRIGGER set_shares_timestamp
...
DROP INDEX IF EXISTS idx_stars_unique;
DROP INDEX IF EXISTS idx_stars_entity;
...
DROP POLICY IF EXISTS "Users can view their own ratings" ON public.stars;
DROP POLICY IF EXISTS "Users can leave reviews" ON public.stars;
DROP POLICY IF EXISTS "Users can update their review" ON public.stars;
DROP POLICY IF EXISTS "Users can delete their review" ON public.stars;
...
DROP TABLE IF EXISTS public.dao_stats CASCADE;
...
DROP INDEX IF EXISTS idx_mentions_target;
